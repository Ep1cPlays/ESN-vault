...
def register_commands(bot: VaultBot) -> None:
    class GameChallenge(discord.ui.View):
        ...
        async def answer(self, interaction: discord.Interaction) -> None:
            ...
            if button.label != self.correct_answer:
                ...
                return
            try:
                credited = await bot.db.resolve_game(
                    self.player_id,
                    self.access_item,
                    self.reward,
                    boost_item=self.boost_item,
                    server_boost=is_server_booster(interaction)
                )
            ...
            await interaction.response.edit_message(
                embed=vault_embed("Mini-game complete", f"Correct. You earned **{credited:,} {COIN}**."),
                view=self
            )
    ...
    class TradeOffer(discord.ui.View):
        ...
        async def accept(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
            if interaction.user.id != self.recipient_id:
                ...
                return
            try:
                await bot.db.trade_items(
                    self.sender_id,
                    self.recipient_id,
                    self.offered_item,
                    self.offered_quantity,
                    self.requested_item,
            ...
            await interaction.response.edit_message(
                embed=vault_embed(
                    "Trade complete",
                    f"<@{self.sender_id}> traded **{self.offered_quantity}x `{self.offered_item}`** for **{self.requested_quantity}x `{self.requested_item}`**."
                ),
                view=self
            )
        ...
        async def decline(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
            ...
            for child in self.children:
                child.disabled = True
            await interaction.response.edit_message(
                embed=vault_embed("Trade declined", "The offered Trade Seal was not consumed."),
                view=self
            )
    ...
    async def balance(interaction: discord.Interaction, user: discord.User | None = None):
        ...
        embed = vault_embed(
            ...
            f"**Wallet**  {account['wallet']:,} {COIN}\n"
            f"**Bank**  {account['bank']:,} {COIN}\n"
            f"**Total wealth**  {account['wallet'] + account['bank']:,} {COIN}"
        )
    ...
    async def daily(interaction: discord.Interaction):
        streak, reward = await bot.db.claim_reward(
            interaction.user.id,
            kind="daily",
            cooldown=86_400,
            reward=0,
        ...
        await interaction.response.send_message(
            embed=vault_embed("Daily claimed", f"You received **{reward:,} {COIN}**. Streak: **{streak}** days.")
        )

    @bot.tree.command(name="weekly", description="Claim your weekly ES Coins")
    @app_commands.checks.cooldown(1, 5 * 60, key=lambda i: i.user.id)
    async def weekly(interaction: discord.Interaction):
        _, reward = await bot.db.claim_reward(
            interaction.user.id,
            kind="weekly",
            cooldown=604_800,
            reward=2_000,
        ...
        await interaction.response.send_message(
            embed=vault_embed("Weekly claimed", f"You received **{reward:,} {COIN}**.")
        )

    @bot.tree.command(name="work", description="Work for a random ES Coin reward")
    async def work(interaction: discord.Interaction):
        reward, cooldown = await bot.db.perform_work(
            interaction.user.id,
            random.randint(75, 225),
            server_boost=is_server_booster(interaction)
        )
        await interaction.response.send_message(
            embed=vault_embed("Shift complete", f"You earned **{reward} {COIN}**. Next shift: **{cooldown}s**.")
        )
    ...
    async def use(interaction: discord.Interaction, item_id: str):
        ...
        await interaction.response.send_message(
            embed=vault_embed("Item used", f"Used `{item_id.lower()}`. {detail}")
        )
    ...
    async def start_game(
        ...
        *,
        ...
        answers: list[str],
        correct_answer: str,
        boost_item: str | None = None
    ) -> None:
        if not await bot.db.effect_is_ready(interaction.user.id, access_item):
            raise EconomyError(
                f"Use a {access_item.replace('-', ' ').title()} before starting this mini-game."
            )
        view = GameChallenge(
            interaction.user.id,
            access_item,
        ...
        await interaction.response.send_message(
            embed=vault_embed(
                "ESN Vault mini-game",
                f"{question}\n\nUse the buttons below. A correct answer consumes your pass and awards ES Coins."
            ),
            view=view
        )
    ...
    async def trivia(interaction: discord.Interaction):
        await start_game(
            interaction,
            access_item="trivia-ticket",
            reward=150,
            question="Which command sends ES Coins to another user?",
            answers=["/pay", "/withdraw", "/shop"],
            correct_answer="/pay",
            boost_item="focus-tonic"
        )
    ...
    async def scavenge(interaction: discord.Interaction):
        await start_game(
            interaction,
            access_item="scavenger-kit",
            reward=175,
            question="Which item shortens your /work cooldown?",
            answers=["Energy Drink", "Trade Seal", "Vault Pass"],
            correct_answer="Energy Drink",
            boost_item="explorer-map"
        )
    ...
    async def riddle(interaction: discord.Interaction):
        await start_game(
            interaction,
            access_item="riddle-key",
            reward=200,
            question="I am used before an item trade can be completed. What am I?",
            answers=["Trade Seal", "Coin Magnet", "Focus Tonic"],
            correct_answer="Trade Seal"
        )
    ...
    async def vault_dive(interaction: discord.Interaction):
        await start_game(
            interaction,
            access_item="vault-pass",
            reward=300,
            question="What is the virtual currency stored in ESN Vault?",
            answers=["ES Coins", "Vault Gems", "Real Dollars"],
            correct_answer="ES Coins"
        )
    ...
    async def trade(
        ...
        user: discord.User,
        offered_item: str,
        offered_quantity: app_commands.Range[int, 1, 100],
        requested_item: str,
        requested_quantity: app_commands.Range[int, 1, 100]
    ):
        ...
        await interaction.response.send_message(
            embed=vault_embed(
                "Trade offer",
                f"{user.mention}, <@{interaction.user.id}> offers "
                f"**{offered_quantity}x `{offered_item.lower()}`** for "
                f"**{requested_quantity}x `{requested_item.lower()}`**."
            ),
            view=view
        )
    ...
    async def interest(interaction: discord.Interaction):
        reward = await bot.db.claim_interest(
            interaction.user.id,
            server_boost=is_server_booster(interaction)
        )
        await interaction.response.send_message(
            embed=vault_embed("Bank interest claimed", f"Your bank earned **{reward:,} {COIN}** in virtual interest.")
        )
    ...
    async def transactions(interaction: discord.Interaction):
        ...
        text = "\n".join(
            f"**{row['kind'].replace('_', ' ').title()}**: {row['amount']:+,} {COIN} - {row['note'] or 'No note'}"
            for row in rows
        ) or "No transactions yet."
        await interaction.response.send_message(
            embed=vault_embed("Recent transactions", text),
            ephemeral=True
        )
    ...
    async def stats(interaction: discord.Interaction):
        ...
        text = (
            f"**Coins earned:** {account['earned_coins']:,} {COIN}\n"
            f"**Work shifts:** {account['work_count']:,}\n"
            f"**Purchases:** {account['purchase_count']:,}\n"
            f"**Payments sent:** {account['payment_count']:,}"
        )
        await interaction.response.send_message(
            embed=vault_embed("Economy statistics", text)
        )
    ...
    async def pay(
        interaction: discord.Interaction,
        user: discord.User,
        amount: app_commands.Range[int, 1, 2_147_483_647]
    ):
        await bot.db.transfer(interaction.user.id, user.id, amount)
        await interaction.response.send_message(
            embed=vault_embed("Payment sent", f"Sent **{amount:,} {COIN}** to {user.mention}.")
        )
    ...
    async def deposit(
        ...
        await interaction.response.send_message(
            embed=vault_embed("Deposited", f"Moved **{amount:,} {COIN}** into your bank.")
        )
        await notify_guild_owner(
            interaction,
            "Vault deposit",
    ...
    async def withdraw(
        ...
        await interaction.response.send_message(
            embed=vault_embed("Withdrawn", f"Moved **{amount:,} {COIN}** to your wallet.")
        )
    ...
    async def profile(
        ...
        await interaction.response.send_message(
            embed=vault_embed(
                ...
                f"Daily streak: **{account['daily_streak']}** days\n"
                f"Wealth: **{account['wallet'] + account['bank']:,} {COIN}**"
            )
        )

    @bot.tree.command(name="leaderboard", description="View the wealth leaderboard")
    async def leaderboard(interaction: discord.Interaction):
        rows = await bot.db.top_accounts()
        text = "\n".join(
            f"**{index}.** <@{row['user_id']}> — "
            f"{row['wallet'] + row['bank']:,} {COIN}"
            for index, row in enumerate(rows, 1)
        ) or "No vault accounts yet."
        await interaction.response.send_message(
            embed=vault_embed("Wealth leaderboard", text)
        )
    ...
    async def inventory(interaction: discord.Interaction):
        ...
        text = "\n".join(
            f"`{row['item_id']}` x{row['quantity']}"
            for row in rows
        ) or "Your inventory is empty."
        await interaction.response.send_message(
            embed=vault_embed("Inventory", text)
        )
    ...
    async def shop(interaction: discord.Interaction):
        ...
        assert bot.db.connection
        cursor = await bot.db.connection.execute(
            "SELECT * FROM shop_items WHERE enabled=1 "
            "AND (guild_id IS NULL OR guild_id=?) "
            "ORDER BY guild_id NULLS FIRST, name",
            (guild_id,)
        )
        rows = await cursor.fetchall()
        text = "\n".join(
            f"{row['emoji']} **{row['name']}** (`{row['item_id']}`) — "
            f"{row['price']:,} {COIN} | stock: "
            f"{'unlimited' if row['stock'] == -1 else row['stock']}"
            for row in rows
        ) or "The shop is empty."
        await interaction.response.send_message(
            embed=vault_embed("Shop", text)
        )

    @bot.tree.command(name="buy", description="Buy an item from the shop")
    async def buy(interaction: discord.Interaction, item_id: str):
        assert bot.db.connection
        item = await bot.db._fetchone(
            "SELECT * FROM shop_items WHERE item_id=? AND enabled=1 "
            "AND (guild_id IS NULL OR guild_id=?)",
            (item_id.lower(), interaction.guild_id)
        )
        if not item:
            ...
            await interaction.response.send_message(
                embed=vault_embed(
                    "Market purchase",
                    f"You bought `{item_id.lower()}` for **{price:,} {COIN}**."
                )
            )
            await notify_guild_owner(
                interaction,
        ...
        if item and item["role_id"]:
            ...
            if not role or not interaction.guild.me.guild_permissions.manage_roles or role >= interaction.guild.me.top_role:
                raise EconomyError(
                    "That role reward is currently unavailable; you were not charged."
                )

        item = await bot.db.buy(
            interaction.user.id,
            item_id.lower(),
            guild_id=interaction.guild_id
        )
        ...
        if item["role_id"]:
            try:
                await interaction.user.add_roles(
                    interaction.guild.get_role(item["role_id"])
                )
            except discord.HTTPException:
                await bot.db.refund_purchase(
                    interaction.user.id,
                    item,
                    reason=f"failed role reward {item_id}"
                )
                raise EconomyError(
                    "The role could not be granted; your purchase was refunded."
                )

        await interaction.response.send_message(
            embed=vault_embed(
                "Purchase complete",
                f"You bought {item['emoji']} **{item['name']}** for "
                f"**{item['price']:,} {COIN}**."
            )
        )
        await notify_guild_owner(
            ...
            "Shop purchase",
            f"**Member:** {interaction.user.mention}\n"
            f"**Item:** {item['emoji']} **{item['name']}** (`{item['item_id']}`)\n"
            f"**Price:** {item['price']:,} {COIN}"
        )
    ...
    async def market(interaction: discord.Interaction):
        ...
        await bot.db.connection.execute(
            "INSERT OR IGNORE INTO market(item_id,price,updated_at) "
            "VALUES('vault-token',500,?)",
            (int(time.time()),)
        )
        await bot.db.connection.commit()
        cursor = await bot.db.connection.execute(
            "SELECT * FROM market ORDER BY item_id"
        )
        rows = await cursor.fetchall()
        text = "\n".join(
            f"`{row['item_id']}` — {row['price']:,} {COIN}"
            for row in rows
        )
        await interaction.response.send_message(
            embed=vault_embed(
    ...
    async def sell(interaction: discord.Interaction, item_id: str):
        price = await bot.db.sell_market_item(
            interaction.user.id,
            item_id.lower()
        )
        await interaction.response.send_message(
            embed=vault_embed(
                "Market sale",
                f"Sold `{item_id.lower()}` for **{price:,} {COIN}**."
            )
        )
        await notify_guild_owner(
            interaction,
    ...
    async def rep(interaction: discord.Interaction, user: discord.User):
        if user.id == interaction.user.id:
            raise EconomyError("You cannot give yourself reputation.")
        await interaction.response.send_message(
            embed=vault_embed(
                "Reputation",
                f"{user.mention} received a reputation point from you."
            )
        )
    ...
    async def achievements(interaction: discord.Interaction):
        ...
        collector = (
            "Unlocked - 5% bonus on /daily, /weekly, and /work earnings."
            if "coin-collector" in unlocked
            else "Locked - earn 1,000 ES Coins from Vault rewards."
        )
        await interaction.response.send_message(
            embed=vault_embed(
                "Achievements",
    ...
    async def quests(interaction: discord.Interaction):
        await interaction.response.send_message(
            embed=vault_embed(
                "Quests",
                "Daily: work 3 times for 300 coins. Weekly: make 5 payments for 1,000 coins."
            )
        )

    async def global_owner(interaction: discord.Interaction) -> bool:
        return interaction.user.id in bot.settings.owner_ids

    @bot.owner_shop_group.command(name="add", description="Add a global shop item")
    @owner_only(bot)
    async def owner_shop_add(
        interaction: discord.Interaction,
        name: str,
        description: str,
        price: app_commands.Range[int, 0, 2_147_483_647],
        item_type: Literal['collectible', 'role', 'cosmetic', 'consumable', 'special'],
        stock: int,
        emoji: str,
        image: str | None = None,
        ...
        if stock < -1:
            raise EconomyError("Stock must be -1 (unlimited) or zero and above.")
        item_id = "global-" + "-".join(name.lower().split())[:40]
        now = int(time.time())
        assert bot.db.connection
        await bot.db.connection.execute(
            "INSERT INTO shop_items(item_id,guild_id,name,description,price,item_type,stock,emoji,image_url,role_id,created_at,updated_at) "
            "VALUES(?,NULL,?,?,?,?,?,?,?,?,?,?)",
            (
                item_id,
                name,
        ...
        await bot.db.connection.commit()
        await interaction.response.send_message(
            embed=vault_embed("Global item added", f"Created `{item_id}`."),
            ephemeral=True
        )
    ...
    async def owner_shop_remove(interaction: discord.Interaction, item_id: str):
        class Confirm(discord.ui.View):
            ...
            async def confirm(self, button: discord.ui.Button, click: discord.Interaction):
                if click.user.id not in bot.settings.owner_ids:
                    return await click.response.send_message(
                        "Owner only.",
                        ephemeral=True
                    )
                assert bot.db.connection
                await bot.db.connection.execute(
                    "DELETE FROM shop_items WHERE item_id=? AND guild_id IS NULL",
                    (item_id,)
                )
                await bot.db.connection.commit()
                button.disabled = True
                await click.response.edit_message(
                    content="Global item deleted.",
                    view=self
                )

        await interaction.response.send_message(
            "Delete this global item?",
            view=Confirm(),
            ephemeral=True
        )
    ...
    async def owner_shop_list(interaction: discord.Interaction):
        assert bot.db.connection
        cursor = await bot.db.connection.execute(
            "SELECT * FROM shop_items WHERE guild_id IS NULL ORDER BY name"
        )
        rows = await cursor.fetchall()
        text = "\n".join(
            f"{row['emoji']} **{row['name']}** (`{row['item_id']}`) - "
            ...
            for row in rows
        ) or "The global shop is empty."
        await interaction.response.send_message(
            embed=vault_embed("Global shop", text),
            ephemeral=True
        )
    ...
    def make_owner_shop_command(action: str):
        async def managed(
            ...
            price: app_commands.Range[int, 0, 2_147_483_647] | None = None,
            emoji: str | None = None
        ):
            assert bot.db.connection

            if action == "stock" and amount < -1:
                raise EconomyError("Stock must be -1 or zero and above.")
            ...
            if action == "edit":
                ...
                if not updates:
                    raise EconomyError("Provide at least one field to edit.")

                values = [*updates.values(), int(time.time()), item_id]

                await bot.db.connection.execute(
                    f"UPDATE shop_items SET "
                    f"{', '.join(f'{key}=?' for key in updates)}, "
                    f"updated_at=? WHERE item_id=? AND guild_id IS NULL",
                    values
                )

            elif action == "stock":
                await bot.db.connection.execute(
                    "UPDATE shop_items SET stock=?,updated_at=? "
                    "WHERE item_id=? AND guild_id IS NULL",
                    (amount, int(time.time()), item_id)
                )

            else:
                await bot.db.connection.execute(
                    "UPDATE shop_items SET enabled=?,updated_at=? "
                    "WHERE item_id=? AND guild_id IS NULL",
                    (1 if action == "enable" else 0, int(time.time()), item_id)
                )

            await bot.db.connection.commit()
            await interaction.response.send_message(
                embed=vault_embed("Global shop updated", f"`{item_id}` updated."),
                ephemeral=True
            )
    ...
    for action in ("edit", "stock", "enable", "disable"):
        command = app_commands.Command(
            name=action,
            description=f"{action.title()} a global item",
            callback=make_owner_shop_command(action)
        )
    ...
    async def economy_give(
        ...
        await interaction.response.send_message(
            embed=vault_embed(
                "Economy updated",
                f"Gave {user.mention} **{amount:,} {COIN}**."
            ),
            ephemeral=True
        )
    ...
    async def economy_remove(
        ...
        await interaction.response.send_message(
            embed=vault_embed(
                "Economy updated",
                f"Removed **{amount:,} {COIN}** from {user.mention}."
            ),
            ephemeral=True
        )
    ...
    async def economy_reset(interaction: discord.Interaction, user: discord.User):
        account = await bot.db.account(user.id)
        await bot.db.change_balance(
            user.id,
            -account["wallet"],
            -account["bank"],
    ...
    async def economy_set(
        interaction: discord.Interaction,
        user: discord.User,
        amount: app_commands.Range[int, 0, 2_147_483_647]
    ):
        account = await bot.db.account(user.id)
        await bot.db.change_balance(
            user.id,
        ...
        await interaction.response.send_message(
            embed=vault_embed(
                "Economy updated",
                f"Set {user.mention}'s wallet to **{amount:,} {COIN}**."
            ),
            ephemeral=True
        )
    ...
    async def server_shop_add(
        ...
        if stock < -1:
            raise EconomyError("Stock must be -1 or zero and above.")
        item_id = f"{interaction.guild_id}-" + "-".join(name.lower().split())[:35]
        now = int(time.time())
        assert bot.db.connection
        await bot.db.connection.execute(
            "INSERT INTO shop_items(item_id,guild_id,name,description,price,item_type,stock,emoji,created_at,updated_at) "
            "VALUES(?,?,?,?,?,?,?,?,?,?)",
            (
                item_id,
        ...
        await bot.db.connection.commit()
        await interaction.response.send_message(
            embed=vault_embed("Server item added", f"Created `{item_id}`."),
            ephemeral=True
        )
    ...
    async def server_shop_remove(interaction: discord.Interaction, item_id: str):
        assert bot.db.connection
        await bot.db.connection.execute(
            "DELETE FROM shop_items WHERE item_id=? AND guild_id=?",
            (item_id, interaction.guild_id)
        )
        await bot.db.connection.commit()
        await interaction.response.send_message(
            embed=vault_embed("Server item removed", f"Removed `{item_id}`."),
            ephemeral=True
        )

    @bot.server_shop_group.command(name="edit", description="Edit a server shop item")
    @admin_only()
    async def server_shop_edit(
        interaction: discord.Interaction,
        item_id: str,
        name: str | None = None,
        description: str | None = None,
        ...
        await bot.db.connection.execute(
            f"UPDATE shop_items SET "
            f"{', '.join(f'{key}=?' for key in updates)}, "
            f"updated_at=? WHERE item_id=? AND guild_id=?",
            values
        )
        await bot.db.connection.commit()
    ...
    async def server_shop_list(interaction: discord.Interaction):
        assert bot.db.connection
        cursor = await bot.db.connection.execute(
            "SELECT * FROM shop_items WHERE guild_id=? ORDER BY name",
            (interaction.guild_id,)
        )
        rows = await cursor.fetchall()
        text = "\n".join(
            ...
            f"{'unlimited' if row['stock'] == -1 else row['stock']}"
            for row in rows
        ) or "This server shop is empty."
        await interaction.response.send_message(
            embed=vault_embed("Server shop", text),
            ephemeral=True
        )
